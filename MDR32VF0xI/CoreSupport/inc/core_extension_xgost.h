...
#if defined (CORE_EXTENSION_XGOST) && (CORE_EXTENSION_XGOST == 1U)
...
/**
 * @brief  Perform an L-transform used in Kuznechik (GOST R 34.12-2015) for 64-bit parts of 128-bit block.
 * @param  rs1: First input value.
 * @param  rs2: Second input value.
...
/**
 * @brief  Perform an G[k] transform used in Magma (GOST R 34.12-2015) for 64-bit block.
...
/**
 * @brief  Perform byte transpose of registers rs1 and rs2, and return the result to register rd,
...
__STATIC_INLINE uint64_t gost64sb(uint64_t rs1, uint64_t rs2) {
    ...
    // __asm__("gost32sb %0, %1, %2" : "=r"(r[0]) : "r"(r1[0]), "r"(r2[0]));
    // __asm__("gost32sb %0, %1, %2" : "=r"(r[1]) : "r"(r1[1]), "r"(r2[1]));
...
__STATIC_INLINE uint64_t gost64sbi(uint64_t rs1, uint64_t rs2) {
    ...
    const uint32_t *r1 = (const uint32_t *)(&(rs1));
    const uint32_t *r2 = (const uint32_t *)(&(rs2));
    // __asm__("gost32sbi %0, %1, %2" : "=r"(r[0]) : "r"(r1[0]), "r"(r2[0]));
    // __asm__("gost32sbi %0, %1, %2" : "=r"(r[1]) : "r"(r1[1]), "r"(r2[1]));
...
__STATIC_INLINE uint64_t gost64kuld(uint64_t rs1, uint64_t rs2) {
    ...
    const uint32_t *r2 = (const uint32_t *)(&(rs2));
    uint32_t tmp;
    // __asm__("gost32kuld  %0, %1, %2" : "=r"(tmp) : "r"(r2[0]), "r"(r2[1]));
    // __asm__("gost32kuldh %0, %1, %2" : "=r"(r[0]) : "r"(r1[0]), "r"(r1[1]));
    tmp = gost32kuld(r2[0], r2[1]);
    r[0] = gost32kuld(r1[0], r1[1]);
    r[0] ^= tmp;
    // __asm__("gost32kuld  %0, %1, %2" : "=r"(tmp) : "r"(r2[1]), "r"(r[0]));
    // __asm__("gost32kuldh %0, %1, %2" : "=r"(r[1]) : "r"(r1[1]), "r"(r2[0]));
...
__STATIC_INLINE uint64_t gost64kule(uint64_t rs1, uint64_t rs2) {
    ...
    const uint32_t *r2 = (const uint32_t *)(&(rs2));
    uint32_t tmp;
    // __asm__("gost32kule  %0, %1, %2" : "=r"(tmp) : "r"(r2[0]), "r"(r2[1]));
    // __asm__("gost32kuleh %0, %1, %2" : "=r"(r[1]) : "r"(r1[0]), "r"(r1[1]));
    tmp = gost32kule(r2[0], r2[1]);
    r[1] = gost32kuleh(r1[0], r1[1]);
    r[1] ^= tmp;
    // __asm__("gost32kule  %0, %1, %2" : "=r"(tmp) : "r"(r1[1]), "r"(r2[0]));
    // __asm__("gost32kuleh %0, %1, %2" : "=r"(r[0]) : "r"(r[1]), "r"(r1[0]));
...
__STATIC_INLINE uint64_t gost64mgma(uint64_t rs1, uint32_t rs2) {
    ...
    uint32_t *r = (uint32_t *)(&(rd));
    const uint32_t *r1 = (const uint32_t *)(&(rs1));
    // __asm__("gost32mgma %0, %1, %2" : "=r"(r[1]) : "r"(r1[1]), "r"(rs2));
