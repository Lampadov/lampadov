...
/** @addtogroup CORE_BM310S BM-320S Core
 * @{
 */
...
#endif /* #if defined(__FPU_PRESENT) && (__FPU_PRESENT > 0) */
...
#define csr_read(reg) ({ \
    uint_xlen_t tmp; \
    __ASM volatile ("csrr %0, " __ASM_STR(reg) : "=r"(tmp)); \
    tmp; })
 ...
 * @return None.
 */
#define csr_write(reg, val) ({ __ASM volatile("csrw " __ASM_STR(reg) ", %0" ::"r"(val)); })
 ...
 */
#define csr_read_write(reg, val) ({ \
    uint_xlen_t tmp; \
    __ASM volatile ("csrrw %0, " __ASM_STR(reg) ", %1" : "=r"(tmp) : "r"(val)); \
    tmp; })
 ...
 * @return None.
 */
#define csr_set_bits(reg, mask) ({ __ASM volatile("csrrs zero," __ASM_STR(reg) ", %0" ::"r"(mask)); })
 ...
 * @return CSR register value before write.
 */
#define csr_read_set_bits(reg, mask) ({ \
    uint_xlen_t tmp; \
    __ASM volatile ("csrrs %0, " __ASM_STR(reg) ", %1" : "=r"(tmp) : "r"(mask)); \
  tmp; })
 ...
 * @return None.
 */
#define csr_clear_bits(reg, mask) ({ __ASM volatile("csrrc zero," __ASM_STR(reg) ", %0" ::"r"(mask)); })
 ...
 * @return CSR register value before write.
 */
#define csr_read_clear_bits(reg, mask) ({ \
    uint_xlen_t tmp; \
    __ASM volatile ("csrrc %0, " __ASM_STR(reg) ", %1" : "=r"(tmp) : "r"(mask)); \
  tmp; })
