 ...
 * @param  PrivilegeMode: @ref IT_PrivilegeModeIRQ_TypeDef - interrupt privilege mode.
 * @return None.
 */
void IT_GlobalEnableIRQ(IT_PrivilegeModeIRQ_TypeDef PrivilegeMode)
{
    /* Check the parameters. */
    assert_param(IS_IT_PRIVILEGE_MODE(PrivilegeMode));

    if (PrivilegeMode == IT_PRIVILEGE_MODE_IRQ_M) {
        csr_set_bits(CSR_MSTATUS, CSR_MSTATUS_MIE);
    } else if (PrivilegeMode == IT_PRIVILEGE_MODE_IRQ_U) {
        csr_set_bits(CSR_USTATUS, CSR_MSTATUS_UIE);
    }
}
...
void IT_GlobalDisableIRQ(IT_PrivilegeModeIRQ_TypeDef PrivilegeMode)
{
    ...
    if (PrivilegeMode == IT_PRIVILEGE_MODE_IRQ_M) {
        ...
        csr_clear_bits(CSR_USTATUS, CSR_USTATUS_UIE);
    }
}
...
#if defined(USE_MDR32F02)
/**
 * @brief  PLIC interrupt service routine.
 * @param  None.
 * @return None.
 */
 ...
 * @note   If the interrupt is configured in vector mode, then its handler must be specified with __INTERRUPT_MACHINE or __INTERRUPT_USER.
 * @param  None.
 * @return None.
 */
__WEAK __INTERRUPT_MACHINE __TRAP_HANDLER_ALIGNED void Trap_IRQHandler(void)
